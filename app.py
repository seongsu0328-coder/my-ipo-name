        ...
        if stock:
            ...
            # --- Tab 3: 개별 기업 평가 ---
                ...
                if is_data_available:
                    ...
                    # 💡 [상단 메인 화면 고정] 프리미엄 2칸 + 기존 6칸 = 총 8칸 (버튼 바깥에 시원하게 노출!)
                    metrics_row1 = [
                        ("💡 FMP 산출 적정주가(DCF)" if is_ko else "💡 FMP DCF Target", dcf_display), 
                        ("📊 재무 건전성 종합 등급", rating_display),
                    ...
                    for i, (label, value) in enumerate(metrics_row1):
                        bg_style = "background:#f8f9fa;" if i < 2 else "" # 프리미엄 2칸만 아주 살짝 회색 배경으로 강조
                        with r1_cols[i]: st.markdown(f'<div class="custom-metric-box" style="{bg_style}"><div class="custom-metric-label">{label}</div><div class="custom-metric-value">{value}</div></div>', unsafe_allow_html=True)
                    ...
                    # 💡 1. 재무분석 버튼 (내부에는 오직 AI 심층 리포트 텍스트만!)
