...
# 4. 캘린더 (리스트)
    ...
    if not all_df.empty:
        ...
        period = st.radio("조회 기간 설정", ["60일 내 상장예정", "최근 6개월", "전체"], horizontal=True)
...
# 5. 상세 리서치 (3단계)
    ...
    if stock:
        ...
        with tab3:
            ...
            if not is_watched:
                if st.button("⭐ 관심 종목으로 등록하고 상장 알림 받기", use_container_width=True, type="primary", key=f"reg_watch_{sid}"):
                    st.session_state.watchlist.append(sid)
                    st.balloons()
                    st.toast(f"{stock['name']} 보관함 추가 완료!")
                    st.rerun()
            else:
                st.success(f"✅ {stock['name']} 종목이 보관함에 저장되어 있습니다.")
                if st.button("❌ 관심 종목 해제", key=f"del_watch_{sid}"):
