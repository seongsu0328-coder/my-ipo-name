...
# 5. 상세 페이지 (이동 로직 보정 + 디자인 + NameError 방지 통합본)
    ...
    if stock:
        ...
        with tab1:
            ...
            # 각 문서별 설명 통합 및 체크포인트 데이터
            def_meta = {
                "S-1": {
                    "desc": "상장을 위해 최초로 제출하는 서류입니다. **Risk Factors**(위험 요소), **Use of Proceeds**(자금 용도), **MD&A**(경영설명)를 확인할 수 있습니다.",
                    "check": [
                ...
                "S-1/A": {
                    "desc": "공모가 밴드와 주식 수가 확정되는 수정 문서입니다. **Pricing Terms**(공모가 확정)와 **Dilution**(기존 주주 희석률)을 확인할 수 있습니다.",
                    "check": [
                ...
                "F-1": {
                    "desc": "해외 기업이 미국 상장 시 제출하는 서류입니다. **Foreign Risk**(국가 리스크)와 **Accounting**(회계 기준 차이)을 확인할 수 있습니다.",
                    "check": [
                ...
                "FWP": {
                    "desc": "기관 투자자 대상 로드쇼(Roadshow) PPT 자료입니다. **Graphics**(비즈니스 모델 시각화), **Strategy**(미래 성장 동력)를 확인할 수 있습니다.",
                    "check": [
                ...
                "424B4": {
                    "desc": "공모가가 확정된 후 발행되는 최종 문서입니다. **Underwriting**(주관사 참여 여부)과 **Final Price**(최종 공모가)를 확인할 수 있습니다.",
                    "check": [
            ...
            # UI 출력: 서류 설명 통합 출력
            st.markdown(f"#### {topic} 가이드")
            st.write(curr_meta['desc'])
            ...
            if st.button(f"🤖 AI에게 {topic} 정밀 분석 요청"):
                with st.spinner(f"{topic} 데이터를 분석 중입니다..."):
                    analysis_prompt = f"""
                    당신은 전문 분석가입니다. {stock['name']}의 {topic} 서류를 분석해 주세요.
                    주요 지표: {curr_meta['check']}
                    MD&A 분석 및 핵심 리스크 1가지를 포함하여 한국어로 5줄 내외 요약하세요.
                    """
