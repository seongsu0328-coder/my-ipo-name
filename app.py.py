        ...
        with tab_login:
            with st.form("login_form"):
                ...
                if st.form_submit_button("로그인", use_container_width=True, type="primary"):
                    ...
                    if user and str(user.get('pw')) == l_pw:
                        st.session_state.auth_status = 'user'
                        st.session_state.user_info = user
                        st.session_state.page = 'calendar' # 로그인 성공 시 메인으로
                        st.rerun()
                    else:
                        st.error("아이디 또는 비밀번호가 일치하지 않습니다.")
            
            if st.button("구경하기 (게스트)", use_container_width=True):
                st.session_state.auth_status = 'guest'
                st.session_state.user_info = {'role': 'guest', 'id': 'guest'}
                st.session_state.page = 'calendar'
                st.rerun()

        with tab_signup:
            st.info("회원가입 기능 준비 중입니다.")
            # (여기에 필요시 회원가입 로직 추가 가능)
