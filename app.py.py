...
def get_ai_analysis(company_name, topic, points):
    ...
    for i in range(max_retries):
        try:
            ...
            단, **"저는 분석가입니다" 같은 자기소개나 인사말은 절대 하지 말고**, 바로 본론부터 작성하세요.
            
            [내용 구성 - 풍성하게 작성할 것]
            1. 첫 번째 문단: 해당 문서에서 발견된 가장 중요한 투자 포인트를 상세히 서술.
            2. 두 번째 문단: MD&A(경영진 분석)를 통해 본 기업의 실질적 성장 가능성.
            3. 세 번째 문단: 투자자가 반드시 경계해야 할 핵심 리스크 1가지와 대응.
            
            위 내용을 바탕으로 전문적인 어조의 한국어로 작성하세요. (문단별로 5줄이내)
            """
