...
# ==========================================
# 화면 2: 시장 분석 통계 (자동 업데이트)
# ==========================================
if st.session_state.page == 'stats':
    count_this_year, avg_10y = get_market_stats(MY_API_KEY)
    
    st.title("📊 Unicornfinder 시장 분석")
    st.write(f"최신 업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    st.subheader("📈 연도별 IPO 트렌드 (최근 5년)")
    chart_data = pd.DataFrame({
        '연도': ['2020', '2021', '2022', '2023', '2024', f'{datetime.now().year}(현재)'],
...
# ==========================================
    ...
    st.sidebar.title("🦄 Unicornfinder")
