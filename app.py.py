...
제공된 영문 컨텍스트를 분석하여, 읽기 편하고 매끄러운 '기업 분석 리포트'를 작성하세요.

[문장 흐름 개선을 위한 3대 원칙]
1. 주어 반복 금지: 
   - 문장마다 회사 이름을 주어로 시작하지 마세요. (예: 'AGI Inc는~', 'AGI Inc의~' 반복 금지)
   - 첫 문장에서만 사명을 명시하고, 이후에는 '이 회사는', '해당 기업은', '동사는' 등의 표현을 쓰거나 주어를 과감히 생략하세요.
   - 예: "마르시아노 테스타가 창업자이자 최대주주로서 경영권을 행사하며..." (자연스러운 연결)

2. 문체 및 어미:
   - 모든 문장은 '~습니다', '~합니다' 체로 작성하여 부드럽고 전문적인 톤을 유지하세요.

3. 논리적 연결 (글쓰기 구조):
   - 개별 정보를 나열하지 말고 하나의 이야기처럼 연결하세요. 
   - 창업자 배경 -> 사업 모델 -> 재무 지표 -> 성장 전략 순으로 매끄럽게 서술하세요.

4. 금지 사항:
   - 별표(**) 기호 및 한자/외국어 오타 절대 금지. 오직 순수 한글 평문만 사용하세요."""
                },
                {
                    "role": "user", 
                    "content": f"Context:\n{context}\n\nQuery: {query}\n\n위 원칙에 따라 사명 반복을 피하고, 마치 사람이 직접 쓴 분석 기사처럼 문장이 매끄럽게 이어지는 리포트를 작성해 주세요."
                }
            ],
            temperature=0.3 # 약간의 유연한 문장 구사를 위해 0.3으로 조정
        )
