...
3. 한자나 외국어(广, quyet, 普通 등)를 절대 금지하며 순수 한글 경어체만 사용하세요."""
                },
                {
                    "role": "user", 
        ...
        # 모든 줄바꿈을 기준으로 나눈 뒤, 공백을 다 지우고 다시 2칸씩 들여쓰기 적용
        lines = [line.strip() for line in clean_result.split('\n') if line.strip()]
        
        # 3개 문단으로 합치기 (혹시 AI가 더 많이 나눴을 경우 대비)
        if len(lines) > 3:
            # 적절히 배분하여 3덩어리로 합침
            p1 = "  " + " ".join(lines[:len(lines)//3])
            p2 = "  " + " ".join(lines[len(lines)//3 : 2*len(lines)//3])
            p3 = "  " + " ".join(lines[2*len(lines)//3:])
            clean_result = f"{p1}\n\n{p2}\n\n{p3}"
        else:
            # 이미 3문단 이하라면 각각 들여쓰기만 적용
