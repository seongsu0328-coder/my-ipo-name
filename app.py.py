...
with main_area.container():
    ...
    # ---------------------------------------------------------
    # 5. 상세 페이지 (Detail) - 잔상 및 스크롤 해결 버전
    # ---------------------------------------------------------
    elif st.session_state.page == 'detail':
        # [해결책 1] 자바스크립트를 이용해 브라우저 스크롤을 즉시 맨 위로 강제 이동
        # 이 코드가 가장 상단에 있어야 진입하자마자 스크롤이 올라갑니다.
        st.components.v1.html(
        ...
        # ---------------------------------------------------------
        # [해결책 2] 화면 전환을 즉시 인지시키기 위해 제목부터 먼저 그리기
        # 데이터를 가져오는 3~4초 동안 캘린더 잔상이 남지 않도록 빈 화면을 지워버립니다.
        # ---------------------------------------------------------
        st.markdown(f"# 🔍 {stock['name']} ({sid})")
        st.caption("분석 리포트 및 실시간 데이터를 불러오는 중입니다...")
        st.divider()
    
        # 나중에 본문 내용을 채울 빈 컨테이너 예약
        detail_body = st.empty()
    
        # --- [데이터 복구 및 무거운 로직 실행] ---
        # 이제부터 시간이 걸리는 DB 호출과 Spinner를 돌립니다.
        if sid not in st.session_state.user_decisions:
        ...
        # [1] 변수 초기화 및 추가 데이터 로딩 (yfinance 등)
        # 이 안에서 실행되는 무거운 로직들도 이제는 제목 아래 Spinner와 함께 돌아갑니다.
        profile = None
        fin_data = {}
        current_p = 0
        off_val = 0
