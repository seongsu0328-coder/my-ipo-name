...
3. 한자나 외국어(quyet, 普通 등)를 절대 금지하며 순수 한글 경어체만 사용하세요."""
        ...
        # [강력 후처리 단계: 레이아웃 정밀 조정]
        
        # 1. HTML 및 특수문자 세척
        clean_result = html.unescape(raw_result)
        clean_result = clean_result.replace("**", "").replace("#", "").strip()

        # 2. 문단 간격 정규화 (여러 줄의 공백을 하나의 줄바꿈으로 통일)
        # \n이 2개 이상 연속되면 무조건 \n\n(한 줄 띄우기)으로 바꿈
        clean_result = re.sub(r'\n\s*\n+', '\n\n', clean_result)

        # 3. 들여쓰기 정규화 (문단 시작의 과도한 공백을 2칸으로 고정)
        # 줄바꿈 뒤에 오는 모든 공백을 제거하고 다시 2칸씩 삽입
        lines = clean_result.split('\n')
        formatted_lines = []
        for line in lines:
            stripped = line.strip()
            if stripped:
                formatted_lines.append("  " + stripped) # 문단 시작 시 공백 2칸 강제 부여
            else:
        ...
        # 5. 최종 필터링 (한글, 숫자, 공백, 문장부호 및 줄바꿈 보존)
