    ...
    # ---------------------------------------------------------
        ...
        if stock:
            ...
            with tab3:
                ...
                # 🔥 [0.5] 데이터 보강 로직 (sid = stock['symbol']은 상단에 정의됨)
                ...
                with r1_c3:
                    if accruals_status == "Low": status, st_cls = ("✅ " + ("Solid" if st.session_state.lang != 'ko' else "건전"), "st-good")
                    elif accruals_status == "High": status, st_cls = ("🚨 " + ("Caution" if st.session_state.lang != 'ko' else "주의"), "st-hot")
                    else: status, st_cls = ("🔍 N/A", "st-neutral")
                ...
                with st.expander(get_text('expander_academic_analysis'), expanded=False):
                    ...
                    # 🚨 [NameError 해결]: is_ko 변수를 조건문 밖으로 구출!
                    ...
                    if is_data_available:
                        ...
                        if is_ko:
                            ...
                            * Current revenue growth is in the **{growth_status}** stage. According to Ritter, high-growth firms should beware of the 'growth trap' in the coming years.
                            ...
                            * Accruals quality is **{accruals_status}**, implying the risk of earnings management by executives is {"low" if accruals_status == "Low" else "notable"}.
                            """)
                        st.info(f"**{get_text('academic_verdict_label')}** " + (f"학술적 관점에서 본 기업은 **{growth_status}** 성격이 강하며, 정보 불확실성은 일정 부분 해소된 상태입니다." if is_ko else f"Academically, this firm exhibits **{growth_status}** characteristics with manageable uncertainty."))
                    else:
                        # 💡 이제 여기서 is_ko를 써도 에러가 나지 않습니다.
                        st.warning("⚠️ " + (get_text('err_no_biz_info') if not is_ko else "재무 데이터 부재로 정성적 분석이 권장됩니다."))
            
                # [3] 재무자료 상세보기 (다국어 AI 분석 연동)
                with st.expander(get_text('expander_financial_analysis'), expanded=False):
                    if is_data_available:
                        ...
                        st.info(ai_report)
                        st.caption("※ Standard CFA algorithm applied." if not is_ko else "※ 본 분석은 실제 재무 데이터를 기반으로 생성된 표준 CFA 분석 알고리즘에 따릅니다.")
                ...
                with st.expander(get_text('expander_references'), expanded=False):
                    ...
                    ref_list = [
                        {"label": get_text('ref_label_growth'), "title": "The Long-Run Performance of IPOs", "author": "Jay R. Ritter (1991)", "link": "https://scholar.google.com/scholar?q=Jay+R.+Ritter+1991"},
                        {"label": get_text('ref_label_fundamental'), "title": "New Lists: Fundamentals and Survival Rates", "author": "Fama & French (2004)", "link": "https://scholar.google.com/scholar?q=Fama+French+2004"},
                        {"label": get_text('ref_label_accounting'), "title": "Earnings Management and the Long-Run Performance", "author": "Teoh et al. (1998)", "link": "https://scholar.google.com/scholar?q=Teoh+Welch+Wong+1998"}
                    ]
                    for r in ref_list:
                        st.markdown(f"<div class='ref-item'><div style='flex:1;'><div class='ref-badge'>{r['label']}</div><br><a href='{r['link']}' target='_blank' class='ref-title'>📄 {r['title']}</a><div style='font-size: 13px; color: #666;'>{r['author']}</div></div><div style='margin-left: 15px;'><a href='{r['link']}' target='_blank' class='ref-btn'>View ↗</a></div></div>", unsafe_allow_html=True)
