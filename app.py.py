    ...
    with tab1:
        ...
        with tab4:
            ...
            if len(missing_steps) > 0:
                ...
                summary_text = f"""<br>사용자는 해당 기업소개와 뉴스에 대해 <b>{d_news}</b>이라 판단했고, 
주요 공시정보에 대해서는 <b>{d_filing}</b>입니다. 현재 거시경제 상황에 대해서 <b>{d_macro}</b>이라 판단하고 있고, 
현 기업의 가치평가에 대해서는 <b>{d_company}</b>이라고 판단합니다. 현재 최종 판단에 앞서 IPO 당사자들이 제공한 정보들과, 이에 대해 특정한 스탠스를 가지고 쓰여진 기사들, 
