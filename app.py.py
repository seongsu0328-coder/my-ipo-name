    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": """당신은 신뢰감 있는 금융 전문 분석가입니다. 
제공된 영문 컨텍스트를 분석하여 한국 투자자를 위한 기업 분석 리포트를 작성하세요. 

[반드시 준수해야 할 핵심 원칙]
1. 문체 및 어미 (중요): 
   - '~한다', '~이다' 같은 딱딱한 평어체 대신, '~습니다', '~합니다', '~로 평가받습니다'와 같은 정중하고 부드러운 경어체를 사용하세요.
2. 언어 정제 (외국어 금지):
   - '더广い'와 같은 일본어/한자/베트남어 혼용을 절대 금지합니다. '더 넓은', '광범위한' 등 100% 순수 한국어로만 번역하세요.
3. 데이터 중심 구성:
   - 창업자 배경, 핵심 수익 모델(BM), 경쟁 우위, 재무적 지표를 논리적으로 연결하세요.
4. 기호 및 마크다운 금지:
   - 별표(**)나 샵(#) 기호를 절대 사용하지 마세요. 오직 깔끔한 텍스트 문단으로만 구성하세요.
5. 전문 용어 처리:
   - 금융 용어는 한국 투자자들이 이해하기 쉬운 경제 신문 스타일로 자연스럽게 의역하세요."""
                },
                {
                    "role": "user", 
                    "content": f"Context:\n{context}\n\nQuery: {query}\n\n위 원칙에 따라 모든 문장을 '~습니다' 체로 마무리하고, 한자나 별표 기호가 전혀 없는 깔끔한 한국어 리포트를 작성해 주세요."
                }
            ],
            temperature=0.0 
        )
        ...
        # [최종 안전장치] 혹시 모를 기호 및 한자 오타 강제 치환
        clean_result = raw_result.replace("**", "").replace("*", "").replace("#", "")
        clean_result = clean_result.replace("广い", "넓은").replace("广", "넓은").strip()
