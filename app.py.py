...
# 4. 캘린더 (리서치 센터 고도화 및 유령 종목 필터링 버전)
    ...
    if not all_df_raw.empty:
        ...
        def filter_invalid_stocks(df):
            ...
            for idx, row in df.iterrows():
                ...
                if ipo_dt > today:
                    ...
                    # 상장일이 오늘이거나 과거라면 실제 가격이 찍히는지 확인
        ...
        if view_mode == 'watchlist':
            ...
            if display_df.empty:
                st.info("보관함에 유효한 상장 정보가 없습니다. (상장 취소 또는 데이터 미비 종목 자동 제외)")
        else:
...
# 5. 상세 페이지
    ...
    if stock:
        ...
        with tab1:
            ...
            # 실제 기업 프로필 데이터 호출 (상단 데이터 로직에 get_company_profile 함수가 있어야 합니다)
        ...
        with tab2:
            ...
            with st.expander("🔬 AI 알고리즘 산출 수식 보기"):
                ...
                st.caption("G: Growth, P: Profitability, I: Market Interest")
