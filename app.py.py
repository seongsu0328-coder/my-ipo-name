    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": """당신은 월스트리트 출신의 시니어 애널리스트입니다. 
제공된 영문 컨텍스트를 분석하여 한국 투자자를 위한 전문 분석 내용을 작성하세요.

[⚠️ 출력 제한 사항]
- **제목(예: ## 기업 분석 리포트)을 절대 작성하지 마세요.**
- 인사말이나 서론 없이 바로 본론 문단으로 시작하세요.

[반드시 준수해야 할 원칙]
1. 'Bellwether' -> '시장 지표' 또는 '업계 풍향계', 'Crowded market' -> '포화된 시장'으로 의역하세요.
2. '~했습니다', '~로 평가받습니다' 등 신뢰감 있는 문어체를 사용하세요.
3. 핵심 수치나 강점은 **굵게(Bold)** 처리하세요.
4. 한자(拥挤 등)를 절대 사용하지 말고 순수 한국어로만 작성하세요."""
                },
                {
                    "role": "user", 
                    "content": f"Context:\n{context}\n\nQuery: {query}\n\n제목 없이 본문 분석 내용만 바로 작성해줘."
                }
            ],
            temperature=0.2 
        )
        ...
        # 💡 [후처리] 만약 AI가 습관적으로 첫 줄에 제목을 넣었을 경우 첫 줄 제거 로직
        lines = raw_content.split('\n')
        if len(lines) > 1 and ('리포트' in lines[0] or '분석' in lines[0] or '##' in lines[0]):
            return '\n'.join(lines[1:]).strip()
...
# 5. 상세 페이지 (이동 로직 보정 + 디자인 + NameError 방지 통합본)
    ...
    if stock:
        ...
        with tab2:
            ...
            # =================================================================
            # 2. 🇺🇸 거시 시장 지표
            # 기존 st.subheader("미국거시경제 과열 평가") 삭제 후 아래 코드 삽입
