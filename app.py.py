...
def get_unified_tab4_analysis(company_name, ticker):
    ...
    [작성 지침]
    1. **언어**: 반드시 한국어로 답변하세요.
    2. **문체**: 모든 문장이 기업명(예: '동사는', '{company_name}은')으로 시작하지 않도록 주의하세요. 예시: "최근 금융 시장의 트렌드를 선도하며...", "주목할 만한 점은...", "재무적인 측면에서 살펴보면..." 등으로 시작.
    3. **분석 깊이**: 단순 사실 나열이 아닌, 구체적인 수치나 근거를 들어 전문적으로 분석하세요.
    4. **Pros & Cons**: 긍정적 요소(Pros) 2가지와 부정적/리스크 요소(Cons) 2가지를 명확히 구분하여 상세하게 서술하세요.
    5. **Rating**: 전반적인 월가 분위기를 종합하여 반드시 (Strong Buy/Buy/Hold/Sell) 중 하나로 선택하세요.
    6. **Summary**: 전문적인 톤으로 3줄 이내로 핵심만 간결하게 작성하세요. (부채 상환, 합병, 청산 등의 주요 이슈가 있다면 반드시 포함)
    7. **링크 금지**: 답변 텍스트(Summary, Pro_con) 내에는 'Source:', 'http...' 등의 출처 링크를 절대 포함하지 마세요.
